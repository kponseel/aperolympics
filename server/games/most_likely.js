// "Le plus susceptible" — secret vote. 240-prompt bank (merged with the
// former Superlatives bank in v50); a fresh random 10 picked per match.
// End-of-session stats: most-voted MVP, the silver-medal "outsider", and
// the "âmes sœurs du vote" pair (the two players who voted for the same
// target most often). Server is authoritative.
...
const PROMPT_BANK = [
  ...
  // +110 from the merged Superlatives bank (v50): same mechanic (vote a
  // person), framed as "Le/la plus X". Mixed in randomly with the
  // "Qui est le plus susceptible…" phrasings above.
