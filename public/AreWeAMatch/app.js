...
(function () {
  ...
  // ---------- langue ----------
  // La chaîne FRANÇAISE sert de clé. Trois raisons : rien à inventer comme
  // noms de clés, le code reste lisible tel quel, et une traduction manquante
  // retombe sur le français — pas sur un identifiant brut affiché au joueur.
  // Les traductions vivent dans i18n.js, chargé avant ce fichier.
  var lang = "fr";
  // T("…") traduit. T("… %1 …", x) remplit les trous : une phrase coupée en
  // deux par une variable ne peut pas se traduire — l'anglais n'a pas le même
  // ordre de mots. Les trous gardent la phrase entière comme clé.
  function T(s) {
    var d = (lang !== "fr" && window.AM_I18N) ? window.AM_I18N[lang] : null;
    var out = (d && d[s] != null) ? d[s] : s;
  ...
  function connect() {
    ...
    socket.on("pin_needed", function () {
      show("s-pseudo");
      $("amPseudoError").className = "am-error center warn";
      $("amPseudoError").textContent = T("Ce pseudo est libre ! Choisis un code de reprise à 4 chiffres pour te le réserver.");
      var pin = $("amPin"); if (pin) { pin.value = ""; pin.focus(); }
    });
    ...
    socket.on("game_created", function (m) {
      ...
      if (m.test) toast(T("🧪 Partie de test créée"));
    });
    ...
    socket.on("answer_ack", function (m) {
      ...
      if (!m.ok) {
        ...
        if (m.reason === "already_answered") { warn(T("Déjà répondu à cette scène.")); play.index += 1; play.myRank = []; renderScene(); return; }
        if (m.reason === "closed") { warn(T("Cette partie est fermée.")); openGame(play.code); return; }
        warn(T("Réponse non prise en compte, réessaie.")); renderScene(); return;
      }
    ...
    socket.on("unanswer_ack", function (m) {
      if (!m) return;
      if (!m.ok) {
        if (m.reason === "revealed") warn(T("Trop tard : quelqu'un a répondu à cette scène, tu as vu sa réponse."));
        else if (m.reason === "finished") warn(T("Tu as terminé la partie : les réponses sont définitives."));
        else if (m.reason === "not_answered") warn(T("Tu n'as pas encore répondu à cette scène."));
        else warn(T("Impossible de revenir sur cette réponse."));
        return;
      }
      ...
      renderScene();
      toast(T("↩️ Tu peux refaire ton classement."));
    });
    ...
    socket.on("game_closed", function (m) {
      if (!m || !m.ok) { warn(T("Impossible de fermer les inscriptions.")); return; }
      if (m.deleted) { toast(T("Partie de test supprimée")); goHome(); return; }
      toast(T("🔒 Inscriptions fermées — tout le monde peut finir"));
    });
    socket.on("game_reopened", function (m) {
      if (!m || !m.ok) { warn(T("Impossible de rouvrir les inscriptions.")); return; }
    ...
    socket.on("game_deleted", function (m) {
      ...
      var why = m && m.reason;
      var err = $("amDelErr");
      var msg = why === "not_host" ? T("Seul l'hôte peut supprimer la partie.")
        : why === "confirm_required" ? T("Recopie exactement le code de la partie.")
        : why === "unknown_game" ? T("Cette partie n'existe déjà plus.")
    ...
    socket.on("dev_state", function (m) {
      if (!m || m.enabled === false) {
        ...
        devSheetError(T("Mode dev désactivé sur ce serveur (ADMIN_PASSWORD n'est pas défini)."));
        return;
      }
      if (!m.ok) {
        if (m.reason === "bad_token") setDevToken("");
        devSheetError(m.reason === "locked" ? T("Trop d'essais — réessaie dans 15 minutes.")
          : m.reason === "bad_token" ? T("Session dev expirée : retape le mot de passe.")
      ...
      toast(T("🧪 Mode dev activé"));
    });
    socket.on("error_msg", function (m) {
      var code = m && m.msg;
      if (code === "bad_identity" || code === "no_identity") return;
      if (code === "bad_pin") { warn(T("Le code de reprise fait 4 chiffres.")); return; }
      if (code === "not_owner") { warn(T("Ce pseudo appartient à un autre appareil.")); return; }
      if (code === "unknown_game") {
        pendingResults = false; warn(T("Aucune partie avec ce code."));
      ...
      if (code === "slow_down") { warn(T("Trop de codes essayés, attends un peu.")); return; }
      if (code === "closed") { warn(T("Les inscriptions de cette partie sont fermées.")); return; }
      if (code === "dev_locked") { warn(T("Mode dev verrouillé.")); return; }
  ...
  function weakPinMsg(why) {
    if (why === "repete") return T("Quatre fois le même chiffre, c'est le deuxième code que quelqu'un essaie. Choisis-en un autre.");
    if (why === "suite") return T("Une suite de chiffres, c'est trop deviné. Choisis-en un autre.");
    return T("Ce code est l'un des plus utilisés au monde — c'est le premier que quelqu'un essaierait. Choisis-en un autre.");
  }
  function enterPinMode(name, msg) {
    pinMode = true; show("s-pseudo");
    $("amLocked").style.display = "none";
    if (name) $("amName").value = name;
    $("amPinLabel").innerHTML = T("🔒 Code de reprise <span class='am-soft'>(4 chiffres)</span>");
    $("amPseudoError").textContent = msg || "";
    $("amPseudoError").className = "am-error center warn";
    var pin = $("amPin"); if (pin) { pin.value = ""; setTimeout(function () { pin.focus(); }, 50); }
    $("amContinue").textContent = T("Déverrouiller 🔓");
  }
  function submitPseudo() {
    var name = ($("amName").value || "").trim().slice(0, 16);
    var pin = ($("amPin").value || "").trim();
    $("amPseudoError").className = "am-error center";
    if (!name) { $("amPseudoError").textContent = T("Entre ton pseudo."); return; }
    if (!/^\d{4}$/.test(pin)) { $("amPseudoError").textContent = T("Ton code de reprise : 4 chiffres. Il ne sert que si tu changes de téléphone, mais il est obligatoire — c'est lui qui te réserve ton pseudo."); $("amPin").focus(); return; }
    $("amPseudoError").textContent = "";
    ...
    openSheet("🔒 " + (mandatory ? "Choisis ton code de reprise" : T("Ton code de reprise")),
      ...
      function (body) {
        ...
        function go() {
          ...
          if (!/^\d{4}$/.test(pin)) { err.textContent = T("Le code doit faire 4 chiffres."); input.focus(); return; }
  ...
  function renderVersion(app) {
    ...
    ver.textContent = (app && app.version) ? T("v%1 · màj %2", app.version, fmtDay(app.date)) : "";
    ver.title = ((app && app.sha) ? "commit " + app.sha + " · " : "") + T("touche pour le mode dev");
  ...
  function renderHome() {
    ...
    var list = gamesCache || [];
    if (!list.length) {
      wrap.innerHTML = T('<div class="am-empty"><b>Aucune partie pour l\'instant.</b><br>Crée la première et partage son QR — tes amis répondront quand ils auront le temps.<br>Ou rejoins celle de quelqu\'un avec son code à 5 caractères.</div>');
      return;
    }
  ...
  function createGameSheet() {
    ...
    openSheet("➕ Nouvelle partie",
      ...
      T('<p class="am-hint">Tu pourras fermer les inscriptions plus tard : ça empêche de nouveaux joueurs d\'entrer, sans jamais couper ceux qui ont commencé.</p>') +
      T('<button type="button" class="am-primary" id="amFormGo">🎲 Créer la partie</button>'),
      function (body) {
  ...
  function joinSheet() {
    openSheet(T("🔗 Rejoindre une partie"),
      T('<p>Tape le code de la partie (il est sous le QR de ton ami), ou scanne son QR avec l\'appareil photo.</p>') +
      '<input id="amFormCode" class="am-codein" maxlength="8" placeholder="7QX3M" autocomplete="off" autocapitalize="characters" />' +
      T('<button type="button" class="am-primary" id="amFormGo">Ouvrir la partie</button>') +
      '<div class="am-error center" id="amFormErr"></div>',
      function (body) {
        ...
        function go() {
          ...
          if (code.length < 4) { body.querySelector("#amFormErr").textContent = T("Le code fait 5 caractères."); return; }
  ...
  function deleteGameSheet(g) {
    ...
    what += T("<p class='am-hint'>Si tu veux juste qu'elle sorte de ta liste, ferme cette fenêtre et choisis <b>Masquer</b> : la partie continue d'exister pour les autres.</p>");
    if (needCode) {
    ...
    openSheet(T("🗑️ Supprimer la partie"), what, function (body) {
      ...
      go.onclick = function () {
        ...
        if (!socket || !connected) { body.querySelector("#amDelErr").textContent = T("Pas de connexion."); return; }
        go.disabled = true;
  ...
  function renderScene() {
    ...
    if (astuce) {
      ...
      body += T('<div class="am-tip">👆 <b>Touche les 3 réponses dans l\'ordre demandé juste au-dessus</b> — la 1<sup>re</sup> que tu touches prend la place n°&nbsp;1. Pas de chrono : prends ton temps, tu peux fermer et revenir.</div>');
    }
    if (play.submitted) {
    ...
    if (vb) vb.onclick = function () {
      ...
      if (!connected) { warn(T("Pas de connexion là — réessaie dans un instant.")); return; }
      play.submitted = true;
    ...
    if (ub) ub.onclick = function () {
      if (!socket || !connected) { warn(T("Pas de connexion là — réessaie dans un instant.")); return; }
  ...
  function renderResults() {
    ...
    var extras = [];
    if (f.groupSoul) extras.push({ e: "🫂", l: T("L'âme sœur du groupe"), n: f.groupSoul.name, v: f.groupSoul.pct + "%" });
    if (f.freeSpirit) extras.push({ e: "🛸", l: T("L'électron libre"), n: f.freeSpirit.name, v: f.freeSpirit.pct + "%" });
    if (f.opposites) extras.push({ e: "⚔️", l: T("Les opposés"), n: f.opposites.a + " & " + f.opposites.b, v: f.opposites.pct + "%" });
    if (r.moments) {
      if (r.moments.divisive) extras.push({ e: "🔥", l: T("La scène qui divise"), n: r.moments.divisive.q, v: r.moments.divisive.pct + "%" });
      if (r.moments.unanimous) extras.push({ e: "🫂", l: T("La scène unanime"), n: r.moments.unanimous.q, v: T("%1 sur %2", r.moments.unanimous.voters, r.moments.unanimous.voters) });
    ...
    body += backLinkCard(r);
    body += T('<button class="am-ghost" id="amScenes">👀 Scène par scène</button>');
  ...
  function renderScenesSheet(reveals) {
    ...
    }).join("") : T('<p class="am-hint">Réponds d\'abord à quelques scènes.</p>');
    openSheet(T("👀 Scène par scène"), html);
  }
  ...
  function logout() {
    ...
    openSheet(T("🚪 Se déconnecter"),
      T("<p>Ce téléphone va oublier <b>%1</b>. Pour revenir, il faudra ton pseudo <b>et</b> ton code de reprise.</p>", esc(getPseudo())) +
      T("<p class='am-hint'>Rien n'est supprimé : tes parties, tes réponses et tes résultats restent intacts.</p>") +
      T('<button type="button" class="am-primary" id="amLogoutGo">🚪 Me déconnecter</button>') +
  ...
  function openDevSheet() {
    ...
    openSheet("🧪 Mode dev",
      ...
      function (body) {
        ...
        function go() {
          var pw = input.value || "";
          if (!pw) { devSheetError(T("Entre le mot de passe admin.")); return; }
          if (!socket || !connected) { devSheetError(T("Pas de connexion.")); return; }
  ...
  function runInstallPrompt() {
    ...
    if (choice) choice.then(function (r) {
      if (r && r.outcome === "accepted") { installed = true; toast(T("📲 C'est installé — regarde ton écran d'accueil.")); }
      renderInstall();
    }, function () { renderInstall(); });
    else renderInstall();
  }
  function iosInstallSheet() {
    openSheet(T("📲 Sur ton iPhone"),
      T("<p>Safari ne sait pas installer tout seul : trois gestes, une fois pour toutes.</p>") +
  ...
  window.addEventListener("appinstalled", function () {
    ...
    renderInstall();
    toast(T("📲 C'est installé — regarde ton écran d'accueil."));
  });
